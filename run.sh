                ...
                for fast_forward in ${fast_forwards[@]};do                                              # fast_forward
                    ...
                    for p in ${placement[@]};do                                                         # placement 策略
                        for s in ${schedule[@]};do
                            ...
                            python $THIS_DIR/worker.py --master_ip $SCHEDULER_IP --worker_port $WORKER_PORT --trace_name ${job_log} --this-dir ${THIS_DIR} $arg --log_path=$THIS_DIR/${log_name} >$THIS_DIR/${log_name}/worker${WORKER_ID}.out &     # ljx 由于家目录共享，所以加个WORKER_ID区分一下不同worker.out 添加--log_path=$THIS_DIR/${log_name}
