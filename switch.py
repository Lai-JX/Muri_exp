    ...
    def try_cross_node_alloc(self, job, not_place=False):
        ...
        for node in full_node_list:
            if not_place==False:
                ...
                node.alloc_job_res(node.num_gpu, idle_node_cpu, 0, gpu_list, job['job_idx'], 1.0)  # 分配gpu和cpu，将job加入gpu的job_list，利用率也加上去
            else:
            ...
            node.add_network_load(traffic, traffic) # in_load，out_load
        ...
        if not_place==False:                    # 设置job['placement']
        ...
        for node in self.node_list:
            ...
            if (node.check_free_gpus() >= need_gpu) and (node.check_free_cpus() >= need_cpu) and (node.free_mem >= JOBS.worker_mem):
                ...
                if not_place==False:
                    ...
                    traffic = JOBS.create_single_node_placement(job, self.id, node.id, need_gpu, need_cpu, JOBS.worker_mem, gpu_list)   # 创建一个node_dict, 加入job['placement']
                else:
